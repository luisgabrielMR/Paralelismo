...
STRATEGY_CONFIGS = [
    ...
    {"strategy": "multiThreadPerFile", "threads_per_file": 2},
    {"strategy": "multiThreadPerFile", "threads_per_file": 4},
    {"strategy": "multiThreadPerFile", "threads_per_file": 8},
]
...
def main():
    ...
    raw_json_path = output_dir / "resultados_brutos.json"
    raw_csv_path = output_dir / "resultados_brutos.csv"
    by_name_csv_path = output_dir / "medias_por_nome.csv"
    general_csv_path = output_dir / "medias_gerais.csv"
    speedup_csv_path = output_dir / "speedup.csv"
    summary_json_path = output_dir / "resumo_benchmark.json"

    save_raw_json(raw_json_path, metadata, runs)
    save_raw_csv(raw_csv_path, runs)
    ...
    save_average_by_name_csv(by_name_csv_path, averages_by_name)
    ...
    save_general_average_csv(general_csv_path, general_averages)

    speedups = calculate_speedup(general_averages, machine_python())
    save_speedup_csv(speedup_csv_path, speedups)
    ...
    save_summary_json(
        ...
        speedups,
        {
            "resultados_brutos_json": str(raw_json_path),
            "resultados_brutos_csv": str(raw_csv_path),
            "medias_por_nome_csv": str(by_name_csv_path),
            "medias_gerais_csv": str(general_csv_path),
            "speedup_csv": str(speedup_csv_path),
            "resumo_benchmark_json": str(summary_json_path),
        },
    )
...
def choose_dataset():
    ...
    if choice == "1":
        return "pequeno", Path(DATASET_PEQUENO_PATH)

    if choice == "2":
        return "grande", Path(DATASET_GRANDE_PATH)
...
def compile_java():
    ...
    if not java_files:
        raise SystemExit("Erro: nenhum arquivo .java encontrado para compilacao.")

    try:
        completed = subprocess.run(
            [JAVAC_COMMAND, *[str(path) for path in java_files]],
            capture_output=True,
            text=True,
        )
...
def sample_random_names(txt_files, amount):
    ...
    if len(sampled_by_name) < amount:
        all_names = collect_all_non_empty_names(txt_files)
        unique_names = {}
        ...
        sampled = random.sample(list(unique_names.values()), amount)
        return assign_nome_ids(sampled)
    ...
    for sampled_name in sampled_names:
        ...
        for config in STRATEGY_CONFIGS:
            for repetition in range(1, REPETICOES_POR_ESTRATEGIA + 1):
                ...
                run = run_java_benchmark(run_id, dataset_key, sampled_name, config, repetition)
                runs.append(run)
                run_id += 1
...
def create_output_dir():
    ...
    base_dir = Path(RESULTS_DIR) / f"{timestamp}_{safe_machine}"
    output_dir = base_dir
    counter = 2
...
def calculate_group_stats(runs, group_keys, include_name):
    ...
    for run in runs:
        if not run["success"]:
            continue
        if as_float(run["wallTimeMs"]) is None:
            continue
    ...
    for key, grouped_runs in groups.items():
        base = {}
        for field, value in zip(group_keys, key):
            base[field] = value
...
def save_summary_json(path, metadata, runs, general_averages, speedups, generated_files):
    ...
    best_strategy = find_best_strategy(general_averages)
    best_speedup = find_best_speedup(speedups)
    ...
    summary = {
        ...
        "melhorEstrategiaPorTempoMedio": best_strategy,
        "maiorSpeedup": best_speedup,
